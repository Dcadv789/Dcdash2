...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'dashboard_chart_components') THEN
    DROP TABLE dashboard_chart_components;
  END IF;
END $$;

-- Create table with proper foreign key relationship
CREATE TABLE dashboard_chart_components (
  ...
  cor text NOT NULL,
  CONSTRAINT fk_dashboard_config 
    FOREIGN KEY (dashboard_id) 
    REFERENCES dashboard_config(id) 
    ON DELETE CASCADE,
...
-- Criar índices
CREATE INDEX idx_dashboard_chart_components_dashboard_id 
  ON dashboard_chart_components(dashboard_id);
CREATE INDEX idx_dashboard_chart_components_categoria_id 
  ON dashboard_chart_components(categoria_id);
CREATE INDEX idx_dashboard_chart_components_indicador_id 
